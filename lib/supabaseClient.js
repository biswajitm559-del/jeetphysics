...
export const AcademicService = {
  ...
  async createResourceMetadata({
    ...
    if (!paperId && !entranceExamId) throw new Error('Either Paper selection or Entrance Exam selection is required');
    ...
    const payload = {
      ...
      entrance_exam_id: entranceExamId || null,
      entrance_topic_id: entranceTopicId || null,
      resource_type: resourceType,
      title: title.trim(),
      description: description && description.trim() ? description.trim() : null,
  ...
  async updateResourceMetadata(resourceId, updates) {
    ...
    if (updates.entranceTopicId !== undefined) {
      payload.entrance_topic_id = updates.entranceTopicId || null;
    }
  ...
  // ==========================================================================
  // 10. ENTRANCE EXAM PLATFORM (PILLAR B)
  // ==========================================================================

  /**
   * Fetch all active entrance exams
   * @returns {Promise<Array>} List of active entrance exams
   */
  async getActiveEntranceExams() {
    const { data, error } = await supabase
      .from('entrance_exams')
      .select('id, slug, name, short_name, description, icon, display_order, is_active')
      .eq('is_active', true)
      .order('display_order', { ascending: true });

    if (error) {
      console.error('Error fetching active entrance exams:', error.message);
      return [];
    }
    return data || [];
  },
  ...
  /**
   * Fetch a single active entrance exam by slug
   * @param {string} slug - e.g. 'iit-jam-physics'
   * @returns {Promise<Object|null>}
   */
  async getEntranceExamBySlug(slug) {
    if (!slug) return null;
    const { data, error } = await supabase
      .from('entrance_exams')
      .select('id, slug, name, short_name, description, icon, display_order, is_active')
    ...
    if (error) {
      console.error(`Error fetching entrance exam "${slug}":`, error.message);
      return null;
    }
    return data;
  },
  ...
  /**
   * Fetch active topics for an entrance exam
   * @param {string} examId - UUID of the entrance exam
   * @returns {Promise<Array>}
   */
  async getEntranceTopics(examId) {
    if (!examId) return [];
    const { data, error } = await supabase
      .from('entrance_topics')
      .select('id, entrance_exam_id, slug, name, description, display_order, is_active')
      .eq('entrance_exam_id', examId)
      .eq('is_active', true)
      .order('display_order', { ascending: true });

    if (error) {
      console.error('Error fetching entrance topics:', error.message);
      return [];
    }
    return data || [];
  },
  ...
  /**
   * Fetch a single active entrance topic by exam slug and topic slug
   * @param {string} examSlug - e.g. 'iit-jam-physics'
   * @param {string} topicSlug - e.g. 'mechanics'
   * @returns {Promise<Object|null>}
   */
  async getEntranceTopicBySlug(examSlug, topicSlug) {
    if (!examSlug || !topicSlug) return null;
    const exam = await this.getEntranceExamBySlug(examSlug);
    if (!exam) return null;

    const { data, error } = await supabase
      .from('entrance_topics')
      .select('id, entrance_exam_id, slug, name, description, display_order, is_active')
      .eq('entrance_exam_id', exam.id)
      .eq('slug', topicSlug.trim().toLowerCase())
      .eq('is_active', true)
      .maybeSingle();

    if (error) {
      console.error(`Error fetching topic "${topicSlug}" for exam "${examSlug}":`, error.message);
      return null;
    }
    if (!data) return null;
    return { ...data, entrance_exams: exam };
  },

  /**
   * Fetch all published resources for an entrance exam (optionally filtered by type and topic)
   * @param {string} entranceExamId - UUID of the entrance exam
   * @param {string|null} [resourceType=null] - e.g. 'PYQ', 'NOTES'
   * @param {string|null} [entranceTopicId=null] - UUID of the topic
   * @returns {Promise<Array>}
   */
  async getPublishedEntranceResources(entranceExamId, resourceType = null, entranceTopicId = null) {
    if (!entranceExamId) return [];
    let query = supabase
      .from('resources')
      .select(`
        id,
      ...
      `)
      .eq('entrance_exam_id', entranceExamId)
      .eq('is_active', true)
      .eq('publication_status', 'PUBLISHED')
      .order('display_order', { ascending: true })
    ...
    if (entranceTopicId) {
      query = query.eq('entrance_topic_id', entranceTopicId);
    }

    const { data, error } = await query;
    if (error) {
      console.error(`Error fetching resources for entrance exam ${entranceExamId}:`, error.message);
      return [];
    }
    return data || [];
  },
  ...
  /**
   * Fetch entrance resources for admin management
   * @param {string} entranceExamId - UUID of the entrance exam
   * @param {string|null} [typeKey=null] - Resource category filter
   * @param {string|null} [entranceTopicId=null] - Topic filter
   * @returns {Promise<Array>}
   */
  async getEntranceResourcesAdmin(entranceExamId, typeKey = null, entranceTopicId = null) {
    if (!entranceExamId) return [];
    let query = supabase
      .from('resources')
      .select(`
    ...
    if (error) {
      console.error(`Error fetching entrance resources for admin:`, error.message);
      throw error;
    }
    return data || [];
  },
  ...
  // --------------------------------------------------------------------------
  // ENTRANCE EXAM ADMIN CRUD
  // --------------------------------------------------------------------------

  /**
   * Fetch all entrance exams for admin (active and inactive)
   * @returns {Promise<Array>}
   */
  async getEntranceExamsAdmin() {
    const { data, error } = await supabase
      .from('entrance_exams')
      .select('*')
      .order('display_order', { ascending: true });
    ...
    if (error) {
      console.error('Error fetching entrance exams for admin:', error.message);
      throw error;
    }
    return data || [];
  },
  ...
  /**
   * Create a new entrance exam (Admin only)
   */
  async createEntranceExam({ name, shortName, slug, description = null, icon = '🎯', displayOrder = 0, isActive = true }) {
    if (!name || !name.trim()) throw new Error('Entrance exam name is required');
    if (!shortName || !shortName.trim()) throw new Error('Short name is required');
    ...
    if (error) {
      console.error('Error creating entrance exam:', error.message);
      throw error;
    }
    return data;
  },
  ...
  /**
   * Update an existing entrance exam (Admin only)
   */
  async updateEntranceExam(examId, updates) {
    if (!examId) throw new Error('Entrance exam ID is required');

    const payload = { updated_at: new Date().toISOString() };
    if (updates.name !== undefined) {
      if (!updates.name || !updates.name.trim()) throw new Error('Entrance exam name cannot be empty');
      payload.name = updates.name.trim();
    }
    if (updates.shortName !== undefined) {
      if (!updates.shortName || !updates.shortName.trim()) throw new Error('Short name cannot be empty');
      payload.short_name = updates.shortName.trim();
    }
    if (updates.slug !== undefined) {
      if (!updates.slug || !updates.slug.trim()) throw new Error('Slug cannot be empty');
      payload.slug = updates.slug.trim().toLowerCase();
    }
    if (updates.description !== undefined) {
      payload.description = updates.description && updates.description.trim() ? updates.description.trim() : null;
    ...
    if (updates.icon !== undefined) {
      payload.icon = updates.icon && updates.icon.trim() ? updates.icon.trim() : '🎯';
    }
    if (updates.displayOrder !== undefined) payload.display_order = parseInt(updates.displayOrder, 10) || 0;
    if (updates.isActive !== undefined) payload.is_active = Boolean(updates.isActive);
    ...
    if (error) {
      console.error(`Error updating entrance exam ${examId}:`, error.message);
      throw error;
    }
    return data;
  },
  ...
  /**
   * Delete an entrance exam safely with dependency checks (Admin only)
   */
  async deleteEntranceExam(examId) {
    if (!examId) throw new Error('Entrance exam ID is required');

    // Check topic dependencies
    const { count: topicCount, error: topicErr } = await supabase
      .from('entrance_topics')
      .select('id', { count: 'exact', head: true })
      .eq('entrance_exam_id', examId);
    if (topicErr) throw topicErr;
    if (topicCount > 0) {
      throw new Error(`Cannot delete entrance exam: ${topicCount} topic(s) depend on it. Please delete or reassign topics first.`);
    }
    ...
    if (resCount > 0) {
      throw new Error(`Cannot delete entrance exam: ${resCount} resource(s) depend on it. Please deactivate the exam (set is_active = false) instead.`);
    }
    ...
    if (error) {
      console.error(`Error deleting entrance exam ${examId}:`, error.message);
      throw error;
    }
    return true;
  },
  ...
  // --------------------------------------------------------------------------
  // ENTRANCE TOPIC ADMIN CRUD
  // --------------------------------------------------------------------------

  /**
   * Fetch entrance topics for admin (optionally filtered by entrance_exam_id)
   * @param {string|null} [examId=null]
   * @returns {Promise<Array>}
   */
  async getEntranceTopicsAdmin(examId = null) {
    let query = supabase
      .from('entrance_topics')
      .select(`
        id,
    ...
    if (examId) {
      query = query.eq('entrance_exam_id', examId);
    }

    const { data, error } = await query;
    if (error) {
      console.error('Error fetching entrance topics for admin:', error.message);
      throw error;
    }
    return data || [];
  },
  ...
  /**
   * Create a new entrance topic (Admin only)
   */
  async createEntranceTopic({ entranceExamId, name, slug, description = null, displayOrder = 0, isActive = true }) {
    if (!entranceExamId) throw new Error('Parent entrance exam selection is required');
    if (!name || !name.trim()) throw new Error('Topic name is required');
    if (!slug || !slug.trim()) throw new Error('Topic slug is required');

    const { data, error } = await supabase
      .from('entrance_topics')
      .insert([{
        entrance_exam_id: entranceExamId,
        name: name.trim(),
        slug: slug.trim().toLowerCase(),
        description: description && description.trim() ? description.trim() : null,
        display_order: parseInt(displayOrder, 10) || 0,
    ...
    if (error) {
      console.error('Error creating entrance topic:', error.message);
      throw error;
    }
    return data;
  },
  ...
  /**
   * Update an existing entrance topic (Admin only)
   */
  async updateEntranceTopic(topicId, updates) {
    if (!topicId) throw new Error('Entrance topic ID is required');

    const payload = { updated_at: new Date().toISOString() };
    if (updates.entranceExamId !== undefined) payload.entrance_exam_id = updates.entranceExamId;
    if (updates.name !== undefined) {
      if (!updates.name || !updates.name.trim()) throw new Error('Topic name cannot be empty');
      payload.name = updates.name.trim();
    }
    if (updates.slug !== undefined) {
      if (!updates.slug || !updates.slug.trim()) throw new Error('Topic slug cannot be empty');
      payload.slug = updates.slug.trim().toLowerCase();
    }
    if (updates.description !== undefined) {
      payload.description = updates.description && updates.description.trim() ? updates.description.trim() : null;
    ...
    if (error) {
      console.error(`Error updating entrance topic ${topicId}:`, error.message);
      throw error;
    }
    return data;
  },
  ...
  /**
   * Delete an entrance topic safely with dependency checks (Admin only)
   */
  async deleteEntranceTopic(topicId) {
    if (!topicId) throw new Error('Entrance topic ID is required');
    ...
    const { count: resCount, error: resErr } = await supabase
      .from('resources')
      .select('id', { count: 'exact', head: true })
      .eq('entrance_topic_id', topicId);
    if (resErr) throw resErr;
    if (resCount > 0) {
      throw new Error(`Cannot delete entrance topic: ${resCount} resource(s) depend on it. Please deactivate the topic (set is_active = false) instead.`);
    }
    ...
    if (error) {
      console.error(`Error deleting entrance topic ${topicId}:`, error.message);
      throw error;
    }
    return true;
  }
